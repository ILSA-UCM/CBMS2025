...
- **DO NOT** include any explanations, code, or additional text—return **only** the JSON array."""
    ...
    response = client.chat(model='deepseek-r1:1.5b', messages=historial_mensajes+
    [
        {
            'role': 'user',
...
def produceResultado(archivo_salida,lista,tp):
    ...
    for item,value in lista.items():
            ...
            while resultadoJSONAnnonUni is None:
                resultadoJSONAnnonUni, resultadoContent = intentoBase(textOri, historial_mensajes, resultadoJSONAnnon,errorAgregadoA)
